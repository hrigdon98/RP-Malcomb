...
ta_2010 = st_clip(st_transform(filter(ta, is.na(capacity_sensitivity) == F), 3395), st_buffer(st_transform(lhz, 3395), .01)) %>%
  st_transform(4326)
# 222 features 

# making capacity rasters from ta capacities
ta_capacity = st_rasterize(ta_2010[, 'capacity_sensitivity'], blank)
...
vulnerability = function(geo) {
  ...
  final = (60 - geo) * 0.60 + drought * 0.20 + flood * 0.20
}
```
