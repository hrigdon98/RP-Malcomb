...
lhz_data = lhz_data %>%
  ...
  select(LZNAMEEN, PctFoodFromOwnFarm, PctCashFromLabor, PctCashFromCrops)
# NEW!
lhz = lhz %>%
...
dhsclusters_2010 = readRDS(here("data", "raw", "private", "datasets", "MWGE62FL.rds")) %>%
  ...
  st_join(st_transform(select(lhz, FNID, PctFoodFromOwnFarm, PctCashFromLabor, PctCashFromCrops),3395)) %>%
  rename(ta_id = ID_2,
         lhz_id = FNID,
         food = PctFoodFromOwnFarm, # NEW!
         labor = PctCashFromLabor, # NEW!
         cashcrops = PctCashFromCrops, # NEW!
...
lhz_sensitivity <- dhshh_2010 %>%
  left_join(st_drop_geometry(select(dhsclusters_2010, DHSCLUST, ta_id, food, labor, cashcrops)), by = c("HV001" = "DHSCLUST")) %>%
  select(
    HHID,
    ta_id,
    food,
  ...
  mutate(
    foodscore = percent_rank(desc(food)) * 4 + 1,  # assuming greater reliance on one's own farm is worse?
    laborscore = percent_rank(desc(labor)) * 4 + 1, # assuming greater reliance on wages is worse for vulnerability?
